...
class Model(nn.Module):
    def __init__(self, state_size, action_size):
        ...
        # Applied to every frame, like Keras' TimeDistributed.
        ...
        self.lstm = nn.LSTM(64 * math.ceil(height / 4) * math.ceil(width / 4), 512, batch_first=True)
        self.output = nn.Linear(512, action_size)

    def forward(self, states):
        batch, frames, height, width = states.shape
        traces = self.frame_model(states.reshape(batch * frames, 1, height, width)).reshape(batch, frames, -1)
        return self.output(self.lstm(traces)[0][:, -1])


class Agent():
    def __init__(self, state_size, action_size, buffer_max_size, chunk_size, add_flipped, always_explore=False, working_dir='.'):
        file_name = 'drqn'+'_'+str(state_size[2])+'f'
        if add_flipped:
            file_name+='_flip'
    ...
    def _build_model(self):
        model = Model(self.state_size, self.action_size)
    ...
    def _targets(self, batch):
        ...
        next_pred = predict(self.model, next_states[not_done]).max(dim=1).values
