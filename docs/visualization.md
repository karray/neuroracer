...
GzWeb supports orbit, zoom, car framing and Play/Pause. Leave Play/Pause alone
while training, which owns world timing. Neither viewer is part of the RL
control path.
...
(message namespaces, disconnect cleanup, module imports, Play request framing,
initial camera, render throttling) and fails the build if upstream source
